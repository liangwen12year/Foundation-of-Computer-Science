import argparse
import json
parser = argparse.ArgumentParser(description="This is about 'HW2 1.6f' simple DFA implementation. The DFA definition is specified via .dfa file; input string is read from stdin. In non-verbose mode,print Accept or Not ACCEPT as the case may be",epilog="*******use the command: python3 dfa3.py -d m3.dfa -v*******alpabet is {0,1} ") 
...
if (args.file): 
     ...
     while(True):
         ...
         for i in q:
            if(i!="0" and i!="1") : 
               print('Invalid alphabet:'+ i+'      igonre rest of string     ')
	      # print('Invalid alphabet: igonre rest of string')
               flag=0
               break
         ...
         elif((lis[-1] == last[0] or lis[-1] == last[1] or lis[-1] == last[2]) ):
 ...
 with open(args.dfafile) as j_data:
   ...
   while(True):
      ...
      for i in q:
            if(i!="0" and i!="1") : 
               print('Invalid alphabet:'+ i+'      ignore rest of string')
               flag=0
               break
            cu_state=spot(cu_state,i)
